...
export const addNewPost = (postData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post("http://localhost:4000/api/v1/post/new", postData, config);
...
export const getPostsOfFollowing = (page = 1) => async (dispatch) => {
    try {
        ...
        setTimeout(async () => {
            ...
            const { data } = await axios.get(`http://localhost:4000/api/v1/posts?page=${page}`);
...
export const addComment = (postId, comment) => async (dispatch) => {
    try {
        ...
        const config = { header: { "Content-Type": "application/json" } }
        const { data } = await axios.post(`http://localhost:4000/api/v1/post/comment/${postId}`, { comment }, config);
...
export const savePost = (postId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post(`http://localhost:4000/api/v1/post/${postId}`);
...
export const getPostDetails = (postId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`http://localhost:4000/api/v1/post/detail/${postId}`);
